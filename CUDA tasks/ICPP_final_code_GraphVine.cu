...
__global__ void data_structure_init(struct vertex_dictionary_structure *device_vertex_dictionary,
                                    unsigned long *d_vertex_id, unsigned long *d_edge_block_count, unsigned int *d_active_edge_count,
                                    unsigned long *d_last_insert_edge_offset, struct edge_block **d_last_insert_edge_block, struct edge_block **d_edge_block_address,
                                    struct edge_block **d_queue_edge_block_address, unsigned long edge_block_count_device) {
...
__global__ void sssp_kernel_preprocessing_dynamic(unsigned int *d_shortest_path, unsigned long *d_search_flag, unsigned long *d_mutex, unsigned long long *d_sssp_queue_1, unsigned long batch_size, unsigned long *d_source) {
    ...
    if(id < batch_size) {
        ...
        d_sssp_queue_1[id] = d_source[id] - 1;
...
__global__ void sssp_output_frontier_preprocessing_EBC(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long *d_source_degrees, unsigned long long *d_sssp_queue_1, unsigned long long *d_sssp_queue_2, unsigned long long *d_sssp_output_frontier_offset, unsigned long *d_csr_offset, unsigned long *d_prefix_sum_edge_blocks) {
    ...
    if(id < d_prev_thread_count) {
        ...
        // if(device_vertex_dictionary->edge_block_count[source] == 0)
        //     printf("Hit 0 degree\n");
...
__global__ void sssp_output_frontier_offset_calculation (unsigned long *d_source_degrees, unsigned long *d_csr_offset, unsigned long long *d_sssp_queue_1, unsigned long long *d_sssp_queue_2, unsigned long long *d_sssp_output_frontier_offset, unsigned long long *d_sssp_e_threads) {
    ...
    *d_sssp_e_threads = d_sssp_output_frontier_offset[d_prev_thread_count];
    // printf("%lu threads in the frontier (device)\n", d_prev_thread_count);
...
__global__ void sssp_kernel_VC_iterative(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, unsigned long batch_size, unsigned int *d_shortest_path, unsigned long *d_source_degrees, unsigned long *d_csr_offset, unsigned long *d_csr_edges, unsigned long *d_prefix_sum_edge_blocks, unsigned long *d_source_vector, unsigned long *d_TC_edge_vector, unsigned long *d_source_vector_1, unsigned long *d_mutex, unsigned long long *d_sssp_queue_1, unsigned long long *d_sssp_queue_2, unsigned long long *d_dp_thread_count) {
    ...
    if((id < d_prev_thread_count)) {
        ...
        long long top = -1;
        const unsigned long stack_size = 2000;
        ...
        while((top > -1) || (curr_node != NULL)) {
            ...
            if(curr_node) {
                ...
                if((d_shortest_path[source] != 4294967295)) {
                    ...
                    for(unsigned long index = 0 ; index < EDGE_BLOCK_SIZE ; index++) {
                        ...
                        if(curr_node->edge_block_entry[index].destination_vertex == 0)
                            break;
                        else if(curr_node->edge_block_entry[index].destination_vertex == INFTY)
                            continue;
                        ...
                        if(d_shortest_path[source] + 1 < d_shortest_path[destination]) {
                            ...
                            // if(d_shortest_path[destination] == 0)
                            //     printf("ID=%lu, source=%lu, d_shortest_path_source=%lu, final value=%lu\n", id, source, d_shortest_path[source], d_shortest_path[source] + 1);
...
__global__ void  sssp_kernel_EBC(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, unsigned long edge_size, unsigned long batch_size, unsigned int *d_shortest_path, unsigned long *d_source_degrees, unsigned long *d_csr_offset, unsigned long *d_csr_edges, unsigned long *d_prefix_sum_edge_blocks, unsigned long *d_source_vector, unsigned long *d_TC_edge_vector, unsigned long *d_source_vector_1, unsigned long *d_mutex, unsigned long long *d_sssp_queue_1, unsigned long long *d_sssp_queue_2, unsigned long long *d_dp_thread_count, unsigned long long *d_sssp_output_frontier_offset, unsigned long thread_multiplier) {
    ...
    if((id < (d_sssp_output_frontier_offset[d_prev_thread_count] * thread_multiplier))) {
        ...
        //     if(d_sssp_output_frontier_offset[i] < id)
        //         break;
        ...
        if((d_shortest_path[source] != 4294967295)) {
            ...
            // unsigned long edge_block_index = floor(double(index_counter) / EDGE_BLOCK_SIZE);
            // unsigned long edge_block_index = floor(double(index_counter) / EDGE_BLOCK_SIZE);
            // unsigned long edge_entry_index = 0;
            unsigned long edge_entry_index = ((id) % thread_multiplier);
            unsigned long start_index = edge_entry_index * 10;
            unsigned long end_index = start_index + 10;
            // if(end_index > EDGE_BLOCK_SIZE)
            ...
            if((root != NULL) && (index_counter < device_vertex_dictionary->edge_block_count[source])) {
                ...
                // goto end;
                // if(id < 50)
                printf("Hit here, id=%lu, source=%lu, source_index=%lu, index_counter=%lu, edge_blocks=%lu, curr=%lu, next=%lu\n", id, source, source_index, index_counter, device_vertex_dictionary->edge_block_count[source], d_sssp_output_frontier_offset[source_index], d_sssp_output_frontier_offset[source_index + 1]);
                goto end;
            }
            for(unsigned long index = start_index ; (index < end_index) ; index++) {
                // for(unsigned long index = 0 ; (index < EDGE_BLOCK_SIZE) ; index++) {
                ...
                if(root->edge_block_entry[index].destination_vertex == 0)
                    break;
                if(root->edge_block_entry[index].destination_vertex == INFTY)
                    continue;
                destination = root->edge_block_entry[index].destination_vertex - 1;
                // curr_index++;
...
__global__ void print_sssp_values(unsigned int *d_shortest_path, unsigned long vertex_size) {
    ...
    unsigned long infty_count = 0;
    for(unsigned long i = 0 ; i < vertex_size ; i++) {

        if(d_shortest_path[i] == 4294967295)
            infty_count++;

    }

    printf("Infinity values are %lu out of %lu\n", infty_count, vertex_size);
...
int main(void) {
    ...
    if(choice == 2) {
        ...
        fullTime += ((double) al_time/CLOCKS_PER_SEC * 1000);       // For compaction
        ...
        // // cudaDeviceSynchronize();
        // // kernel_delete = clock() - kernel_delete;
        unsigned long decision_tree_true = (EDGE_BLOCK_SIZE > 40) && (BATCH_SIZE <= 10000000);
        // unsigned long decision_tree_true = 1;
        // if(EDGE_BLOCK_SIZE > 100) {
        if(decision_tree_true) {
            // if((average_degree_batch > 35) && (BATCH_SIZE <= 10000000)) {
            unsigned long thread_count_EBC;
        ...
        fullTime += ((double) delete_time / CLOCKS_PER_SEC * 1000);     // For compaction
    ...
    while(exitFlag) {
        ...
        std::cout << std::endl << "Please enter any of the below options" << std::endl;
        std::cout << "1. Search for and edge" << std::endl;
        std::cout << "2. Delete an edge" << std::endl;
        std::cout << "3. Print Adjacency" << std::endl;
        std::cout << "4. PageRank Calculation" << std::endl;
        std::cout << "5. Static Traingle Counting" << std::endl;
        std::cout << "6. Single-Source Shortest Path";
        std::cout << std::endl << "7. Exit" << std::endl;
        std::cout << "8. Insert Batch" << std::endl;
        std::cout << "9. Delete Batch" << std::endl;
        std::cout << "10. Compaction Vertex Centric" << std::endl;
        std::cout << "11. Static Triangle Count Edge Centric" << std::endl;
        scanf("%lu", &menuChoice);
        ...
        switch(menuChoice) {
                ...
                if(sssp_type == 1) {
                    ...
                    if(next_op == 1) {
                        ...
                        for(unsigned long tt = 0 ; tt < 5 ; tt++) {
                            ...
                            std::cout << std::endl << "Iteration " << tt << std::endl;
                            ...
                            generate_random_batch(h_graph_prop->xDim, BATCH_SIZE, h_source, h_destination, h_source_degrees_new, h_source_degrees, tt + 1);
                            // generate_random_batch_skewed(h_graph_prop->xDim, BATCH_SIZE, h_source, h_destination, h_source_degrees_new, h_source_degrees, kk);
                            std::cout << "Generated random batch" << std::endl;
                            cudaDeviceSynchronize();
                            // generate_csr_batch(vertex_size, edge_size, max_degree, h_source, h_destination, h_csr_offset_new, h_csr_edges_new, h_source_degrees_new, h_edge_blocks_count, h_prefix_sum_edge_blocks_new, h_batch_update_data, BATCH_SIZE, total_batches, kk, space_remaining, &total_edge_blocks_count_batch, &init_time);
                            thrust::exclusive_scan(h_source_degrees.begin(), h_source_degrees.begin() + vertex_size + 1, h_csr_offset_new.begin());
                            cudaDeviceSynchronize();
                            generate_csr_batch_tester(vertex_size, edge_size, max_degree, h_source, h_destination, h_csr_offset_new, h_csr_edges_new, h_source_degrees_new, h_edge_blocks_count, h_prefix_sum_edge_blocks_new, h_batch_update_data, batch_size, total_batches, tt + 1, space_remaining, &total_edge_blocks_count_batch, &init_time);
                            std::cout << "Generated CSR of random batch" << std::endl;
                            cudaDeviceSynchronize();
                            ...
                            // Inter-batch deduplication disabled for now
                            if(0 && (EDGE_BLOCK_SIZE > 35) && (BATCH_SIZE <= 10000000)) {
                                // if((average_degree_batch > 35) && (BATCH_SIZE <= 10000000)) {
                            ...
                            // thread_blocks = ceil(double(h_graph_prop->xDim) / THREADS_PER_BLOCK);
                            thread_blocks = ceil(double(batch_size) / THREADS_PER_BLOCK);
                            batched_edge_inserts_EC<<< thread_blocks, THREADS_PER_BLOCK>>>(device_edge_block, d_edge_blocks_count_pointer, total_edge_blocks_count_batch, vertex_size, edge_size, d_prefix_sum_edge_blocks_new_pointer, thread_blocks, device_vertex_dictionary, tt + 1, batch_size, start_index, end_index, d_csr_offset_new_pointer, d_csr_edges_new_pointer, d_source_degrees_new_pointer);
                            batched_edge_inserts_EC_postprocessing<<< thread_blocks, THREADS_PER_BLOCK>>>(device_vertex_dictionary, vertex_size, batch_size, d_csr_offset_new_pointer, d_csr_edges_new_pointer, d_source_degrees_new_pointer, d_prefix_sum_edge_blocks_new_pointer, tt + 1);
                            ...
                            do {
                                ...
                                // printf("%lu threads needed\n", (unsigned long)h_dp_thread_count);
                                cudaDeviceSynchronize();
                    ...
                    do {
                        ...
                        // std::cout << h_dp_thread_count << " vertices in frontier" << std::endl;
                    ...
                    if(next_op == 1) {
                        ...
                        for(unsigned long tt = 0 ; tt < 5 ; tt++) {
                            ...
                            // below is worklist based
                            // std::cout << "Thread multiplier is " << thread_multiplier << std::endl;
                            ...
                            do {
                                ...
                                // std::cout << h_dp_thread_count << " vertices in frontier (host)" << std::endl;
                                ...
                                // if(!(h_sssp_e_threads / SSSP_LOAD_FACTOR))
                                //     h_sssp_e_threads = SSSP_LOAD_FACTOR;
                                std::cout << h_sssp_e_threads * thread_multiplier << " launch threads needed" << std::endl;
                                unsigned long thread_blocks = ceil((double(h_sssp_e_threads) * thread_multiplier) / THREADS_PER_BLOCK);
            ...
            case 10 :
                ...
                break;
            case 11:
                // triangleCount(device_vertex_dictionary, h_graph_prop);       // Uncomment this function call for low degree graphs
                triangleCountSorted(device_vertex_dictionary, h_graph_prop);    // Uncomment this function call for high degree graphs
                break;
            default :;
