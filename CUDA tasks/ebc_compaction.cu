...
// below macros are extinct, and will be removed soon
...
__global__ void data_structure_init(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long *d_vertex_id, unsigned long *d_edge_block_count,
                                    unsigned int *d_active_edge_count, unsigned long *d_last_insert_edge_offset,
                                    struct edge_block **d_last_insert_edge_block, struct edge_block **d_edge_block_address, struct edge_block **d_queue_edge_block_address, unsigned long edge_block_count_device) {
    ...
    d_e_queue.rear = -1;
    d_e_queue.count = 0;
    d_e_queue.size = (unsigned int)edge_block_count_device;
...
__global__ void parallel_push_edge_preallocate_list_to_device_queue_v1(struct edge_block* d_edge_preallocate_list, unsigned long total_edge_blocks_count_init, unsigned long device_edge_block_capacity) {
...
// device_vertex_dictionary->edge_block_address[source_vertex], bit_string, length, root, device_vertex_dictionary->last_insert_edge_block[source_vertex], current_edge_block_counter, global_index_counter, active_edge_block_count - 1, active_edge_block_count + new_edge_block_count - 1, source_vertex, id
...
__global__ void batched_edge_inserts_EC_postprocessing(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, unsigned long batch_size, unsigned long *d_csr_offset, unsigned long *d_csr_edges, unsigned long *d_source_degrees, unsigned long *d_prefix_sum_edge_blocks, unsigned long batch_number) {
    ...
    if(id < batch_size) {
        ...
        if(1) {
            ...
            // if(id == 26344527) {
            //     printf("ID=%lu, src=%lu, base=%p\n", id, source_vertex, NULL);

            //     printf("Degree of %lu is %lu, d_csr_offset_index=%lu, index_counter=%lu\n", 328556, d_csr_offset[328557] - d_csr_offset[328556], d_csr_offset[328556], index_counter);
            ...
            if(!index_counter) {
                ...
                if((device_vertex_dictionary->edge_block_address[source_vertex] == NULL) || (batch_number == 0)) {
                    ...
                    // if(base == NULL)
                    //     printf("Null hit during insertion\n");
                    // if(id == 328556)
                    //     printf("ID=%lu, src=%lu, base=%p\n", id, source_vertex, base);
                    ...
                    // if(device_vertex_dictionary->edge_block_address[source_vertex] != NULL) {
                else if ((device_vertex_dictionary->edge_block_address[source_vertex] != NULL) && (batch_number)) {
...
__device__ struct edge_block *search_blocks[1];
...
__global__ void pageRank_kernel_preprocessing_v2(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, unsigned long *d_csr_offset, unsigned long *d_prefix_sum_edge_blocks, unsigned long *d_source_degrees, unsigned long *d_source_vector) {
    ...
    if(id < vertex_size) {
        ...
        // unsigned long vertex_edge_blocks = d_prefix_sum_edge_blocks[id + 1] - d_prefix_sum_edge_blocks[id];
        unsigned long vertex_edge_blocks = device_vertex_dictionary->edge_block_count[id];
        // unsigned long vertex_degree = device;

        // d_source_vector[id] = vertex_edge_blocks * batch_degree;
        d_source_vector[id] = vertex_edge_blocks;
...
__global__ void pageRank_kernel_v2(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, float *d_pageRankVector_1, float *d_pageRankVector_2, unsigned long *d_source_vector) {
    ...
    if((id < d_source_vector[vertex_size])) {
        ...
        float page_factor = d_pageRankVector_1[source_vertex] / device_vertex_dictionary->active_edge_count[source_vertex];
...
__global__ void pageRank_kernel_HD(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, float *d_pageRankVector_1, float *d_pageRankVector_2, unsigned long *d_source_vector) {
...
__global__ void pageRank_kernel_dynamic(struct vertex_dictionary_structure *device_vertex_dictionary, unsigned long vertex_size, unsigned long batch_size, float *d_pageRankVector_1, float *d_pageRankVector_2, unsigned long *d_source_vector, unsigned long *d_csr_offset, unsigned long *d_csr_edges) {
    ...
    if((id < batch_size)) {
        ...
        // unsigned long source_vertex = d_source_vector[id];
        // unsigned long source_vertex = device_binary_search(d_source_vector, id, vertex_size + 1);
        unsigned long source_vertex = d_csr_edges[id] - 1;
...
__global__ void sssp_kernel_preprocessing_dynamic_stage1(unsigned long batch_size) {
    ...
    // d_mutex[0] = 1;
    // tester = 0;
    d_prev_thread_count = batch_size;

}

__global__ void sssp_kernel_preprocessing_dynamic(unsigned int *d_shortest_path, unsigned long *d_search_flag, unsigned long *d_mutex, unsigned long long *d_sssp_queue_1, unsigned long batch_size, unsigned long *d_source) {
...
__global__ void print_pageRank_values(float *d_pageRank_vector, unsigned long vertex_size) {
...
void generate_random_batch_skewed(unsigned long vertex_size, unsigned long batch_size, thrust::host_vector <unsigned long> &h_source, thrust::host_vector <unsigned long> &h_destination, thrust::host_vector <unsigned long> &h_source_degrees_new, thrust::host_vector <unsigned long> &h_source_degrees, unsigned long batch_number) {
    ...
    {
        // EdgeUpdateType edge_update_data;
        unsigned long intermediate = rand() % ((range && (range < (BATCH_SIZE / factor))) ? range : (BATCH_SIZE / factor));
        unsigned long source;
        if(offset + intermediate < vertex_size)
            source = offset + intermediate;
        else
    ...
    {
        // EdgeUpdateType edge_update_data;
        unsigned long intermediate = rand() % (BATCH_SIZE / factor);
        unsigned long source;
        if(offset + intermediate < vertex_size)
            source = offset + intermediate;
        else
...
unsigned long get_edge_block_count_device() {
    ...
    unsigned long edge_blocks_possible = half_device_memory / (sizeof(struct edge_block));
...
__global__ void printQueuePtrs(){
    ...
    d_e_queue.rear %= d_e_queue.size;
    // d_e_queue.front %= EDGE_PREALLOCATE_LIST_SIZE;
    d_e_queue.front %= d_e_queue.size;
    printf("%u\n", d_e_queue.rear);
    printf("%u\n", d_e_queue.front);
}

__global__ void findTotalEdgesandEdgeBlocks(unsigned int totalvertices,
                                            ...
                                            unsigned int *edge_per_vertex_prefixSum,
                                            unsigned int *edge_per_vertex){
    *total_edge_blocks = blocks_per_vertex_prefixSum[totalvertices - 1] + blocks_per_vertex[totalvertices - 1];
    blocks_per_vertex_prefixSum[totalvertices]  = *total_edge_blocks;
    *total_edges = edge_per_vertex_prefixSum[totalvertices - 1] + edge_per_vertex[totalvertices - 1];
    edge_per_vertex_prefixSum[totalvertices] = *total_edges;
}
...
__global__ void recoveredMemoryKernel1(unsigned int* num_blocks_before, double *totalMemoryBeforeCompaction, unsigned int *totalEdgeBlocks,
    ...
    else *num_blocks_before = d_e_queue.size - d_e_queue.front + d_e_queue.rear;
...
__global__ void recoveredMemoryKernel2(unsigned int* num_blocks_before, unsigned int* num_blocks_after){
    ...
    else *num_blocks_after = d_e_queue.size - d_e_queue.front + d_e_queue.rear;
...
__device__ void acquireLock(int *mutex){
    while(atomicCAS(mutex, 0, 1) != 0);
}

__device__ void releaseLock(int *mutex){
    atomicExch(mutex, 0);
}
...
//    printf("%lu\n", total_edge_blocks);
    ...
    //     --last_swap_offset;
    // }
    if(last_swap_offset == 0) last_swap_offset = EDGE_BLOCK_SIZE - 1;
    else --last_swap_offset;
...
//                  if(d_e_queue.rear >= EDGE_PREALLOCATE_LIST_SIZE) printf("%ld\n", id);
                    ...
                    push_index_for_edge_queue %= d_e_queue.size;
...
__global__ void countPerVertexEdgeandEdgeBlock(unsigned int vertices, unsigned int *edge_count_per_vertex, unsigned int *edge_block_count_per_vertex,
                                               struct vertex_dictionary_structure *device_vertex_dictionary){
    unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    edge_count_per_vertex[id] = device_vertex_dictionary->active_edge_count[id];
    edge_block_count_per_vertex[id] = device_vertex_dictionary->edge_block_count[id];
}
...
__global__ void triangleCountingEdgeCentricWithSortedSearch(unsigned int total_edges, unsigned int total_vertices, unsigned int *edge_count_per_vertex_prefixSum, unsigned int* tc_per_vertex,
                                                            struct vertex_dictionary_structure *device_vertex_dictionary){
    unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
...
//    printf("id: %u, Edge destination: %u\n", id, dest);
    ...
    struct edge_block *last_insert_block_for_dest = device_vertex_dictionary->last_insert_edge_block[dest];
    unsigned int last_insert_offset_for_dest = device_vertex_dictionary->last_insert_edge_offset[dest];
    if(last_insert_offset_for_dest == 0) last_insert_offset_for_dest = EDGE_BLOCK_SIZE - 1;
    ...
    while (req_edge_block){
        while (edge_offset_in_edge_block < EDGE_BLOCK_SIZE){
            ...
            if(third_vertex == INFTY) continue; //MOVE TO NEXT EDGE BLOCK
            if(third_vertex == 0) break;
            ...
            while(temp){
                ...
                if(temp == last_insert_block_for_dest) end_idx = last_insert_offset_for_dest;
                else end_idx = EDGE_BLOCK_SIZE - 1;

                while(start_idx <= end_idx){
                    int mid = start_idx + (end_idx - start_idx) / 2;

                    v = (unsigned int) temp->edge_block_entry[mid].destination_vertex - 1;
                    ...
                    else if(v > third_vertex) end_idx = mid - 1;
...
__global__ void sortEdgesWithinEdgeBlock(unsigned int total_vertices, unsigned int* total_edge_blocks,
                                         unsigned int *edge_block_count_per_vertex_prefixSum,
                                         struct vertex_dictionary_structure *device_vertex_dictionary){
    unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
    if(id >= *total_edge_blocks) return;

    // 1. Find source vertex for the edge block using binary search
    unsigned int start = 0;
    unsigned int end = total_vertices;
    unsigned int src = -1;
    ...
    while(start <= end){
        ...
        if(edge_block_count_per_vertex_prefixSum[mid] == id){
            src = mid;
            start = mid + 1;
        }
        else if(edge_block_count_per_vertex_prefixSum[mid] < id){
            src = mid;
            start = mid + 1;
        }
        else end = mid - 1;
    ...
    // 2. Find edge block corresponding to the given thread
    unsigned int edge_block_index = id - edge_block_count_per_vertex_prefixSum[src];
    unsigned long bitString = bit_string_lookup[edge_block_index];
    struct edge_block *req_edge_block = traverse_bit_string(root, bitString);

    // 3. Check whether the curr edge block is last edge block for src vertex or not
    // and adjust the ending index accordingly
    struct edge_block *last_edge_block = device_vertex_dictionary->last_insert_edge_block[src];
    int ending;
    unsigned int last_insert_offset = device_vertex_dictionary->last_insert_edge_offset[src];
    if(req_edge_block == last_edge_block){
        if(last_insert_offset == 0) ending = EDGE_BLOCK_SIZE;
        else ending = last_insert_offset;
    }
    else ending = EDGE_BLOCK_SIZE;

    // 4. Sort data
    int i, key, j;
    for(i = 1; i < ending; ++i){
        key = req_edge_block->edge_block_entry[i].destination_vertex;
        j = i - 1;

        while(j >= 0 && req_edge_block->edge_block_entry[j].destination_vertex > key){
            req_edge_block->edge_block_entry[j + 1].destination_vertex = req_edge_block->edge_block_entry[j].destination_vertex;
            j = j - 1;
        }
        req_edge_block->edge_block_entry[j + 1].destination_vertex = key;
    }
}

void triangleCountSorted(struct vertex_dictionary_structure *device_vertex_dictionary, struct graph_properties *h_graph_prop){
    clock_t start, end;
    clock_t prefix_start, prefix_end;
    float total_preprocess_time = 0.0;
    float tc_kernel_time = 0.0;
    ...
    // 1. Find total edges per vertex in an array and total edge blocks per vertex
    std::cout << "PREPROCESSING BEFORE LAUNCHING TC" << std::endl;
    ...
    cudaMalloc(&edge_count_per_vertex, total_vertices * sizeof(unsigned int));
    cudaMalloc(&edge_block_count_per_vertex, total_vertices * sizeof(unsigned int));

    unsigned int thread_blocks = ceil((double) total_vertices / THREADS_PER_BLOCK);
    prefix_start = clock();
    countPerVertexEdgeandEdgeBlock<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, edge_count_per_vertex, edge_block_count_per_vertex, device_vertex_dictionary);
    cudaDeviceSynchronize();
    prefix_end = clock();

    total_preprocess_time += ((double) (prefix_end - prefix_start)) / CLOCKS_PER_SEC * 1000;

    // 2. Do prefix Sum of the above arrays
    unsigned int *edge_count_per_vertex_prefixSum;
    unsigned int *edge_block_count_per_vertex_prefixSum;
    cudaMalloc(&edge_count_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));
    cudaMalloc(&edge_block_count_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));

    thrust::device_ptr<unsigned int> edge_count_per_vertex_ptr = thrust::device_pointer_cast(edge_count_per_vertex);
    thrust::device_ptr<unsigned int> edge_count_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(edge_count_per_vertex_prefixSum);
    thrust::device_ptr<unsigned int> edge_block_count_per_vertex_ptr = thrust::device_pointer_cast(edge_block_count_per_vertex);
    thrust::device_ptr<unsigned int> edge_block_count_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(edge_block_count_per_vertex_prefixSum);
    cudaDeviceSynchronize();

    prefix_start = clock();
    thrust::exclusive_scan(thrust::device, edge_count_per_vertex_ptr, edge_count_per_vertex_ptr + total_vertices, edge_count_per_vertex_prefixSum_ptr);
    thrust::exclusive_scan(thrust::device, edge_block_count_per_vertex_ptr, edge_block_count_per_vertex_ptr + total_vertices, edge_block_count_per_vertex_prefixSum_ptr);
    cudaDeviceSynchronize();
    prefix_end = clock();
    ...
    findTotalEdgesandEdgeBlocks<<<1,1>>>(total_vertices, total_edge_blocks, total_edges, edge_block_count_per_vertex_prefixSum, edge_block_count_per_vertex, edge_count_per_vertex_prefixSum, edge_count_per_vertex);
    cudaDeviceSynchronize();
    ...
    // 3. Do Sorting of edges within the Edge Blocks in Parallel
    std::cout << "SORTING THE DATA WITHIN THE EDGE BLOCKS" << std::endl;
    thread_blocks = ceil((double) *host_total_edge_blocks / THREADS_PER_BLOCK);
    prefix_start = clock();
    sortEdgesWithinEdgeBlock<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, total_edge_blocks, edge_block_count_per_vertex_prefixSum, device_vertex_dictionary);
    cudaDeviceSynchronize();
    prefix_end = clock();

    total_preprocess_time += ((double) (prefix_end - prefix_start)) / CLOCKS_PER_SEC * 1000;

    // 4. Allocate space for tc
    unsigned int *tc_per_vertex;
    cudaMalloc(&tc_per_vertex, total_vertices * sizeof(tc_per_vertex));
    ...
    // 5. Launch edge centric kernel to calculate TC
    thread_blocks = ceil((double) *host_total_edges / THREADS_PER_BLOCK);
    start = clock();
    triangleCountingEdgeCentricWithSortedSearch<<<thread_blocks, THREADS_PER_BLOCK>>>(*host_total_edges, total_vertices, edge_count_per_vertex_prefixSum, tc_per_vertex, device_vertex_dictionary);
    cudaDeviceSynchronize();
    end = clock();
...
__device__ double my_ceil(double x) {
...
__global__ void findRetainedBlocksAfterCompaction(unsigned int total_vertices,
                                                  unsigned  int *edge_count_per_vertex,
                                                  unsigned int *edge_block_count_per_vertex,
                                                  unsigned int *required_blocks_per_vertex,
                                                  unsigned int *freed_edge_blocks_per_vertex){
    unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
    if(id >= total_vertices) return;

    required_blocks_per_vertex[id] = my_ceil((double) edge_count_per_vertex[id] / EDGE_BLOCK_SIZE);
    freed_edge_blocks_per_vertex[id] = edge_block_count_per_vertex[id] - required_blocks_per_vertex[id];
}

__global__ void printVariable(unsigned int *i){
    printf("%u\n", *i);
}

__global__ void findTotalActiveEdgeandTotalEdgeBlocks(unsigned int total_vertices,
                                                      unsigned int *total_active_edge_count,
                                                      unsigned int *total_edge_block_count,
                                                      unsigned int *total_freed_blocks,
                                                      unsigned int *edge_count_per_vertex,
                                                      unsigned int *edge_count_per_vertex_prefixSum,
                                                      unsigned int *edge_block_count_per_vertex,
                                                      unsigned int *edge_block_count_per_vertex_prefixSum,
                                                      unsigned int *freed_edge_blocks_per_vertex,
                                                      unsigned int *freed_edge_blocks_per_vertex_prefixSum,
                                                      unsigned int *retained_edge_blocks,
                                                      unsigned int *required_edge_blocks_per_vertex,
                                                      unsigned int *required_edge_blocks_per_vertex_prefixSum){
   unsigned int temp = edge_count_per_vertex_prefixSum[total_vertices - 1] +edge_count_per_vertex[total_vertices - 1];
   edge_count_per_vertex_prefixSum[total_vertices] = temp;
   *total_active_edge_count = temp;
//   printf("%u \n", *total_active_edge_count);

    edge_count_per_vertex_prefixSum[total_vertices] = *total_active_edge_count;
    *total_edge_block_count = edge_block_count_per_vertex[total_vertices - 1] + edge_block_count_per_vertex_prefixSum[total_vertices - 1];
    edge_block_count_per_vertex_prefixSum[total_vertices] = *total_edge_block_count;
    // printf("%u ", total_vertices);
    *total_freed_blocks = freed_edge_blocks_per_vertex[total_vertices - 1] + freed_edge_blocks_per_vertex_prefixSum[total_vertices - 1];
    freed_edge_blocks_per_vertex_prefixSum[total_vertices] = *total_freed_blocks;
    *retained_edge_blocks = required_edge_blocks_per_vertex[total_vertices - 1] + required_edge_blocks_per_vertex_prefixSum[total_vertices - 1];
    required_edge_blocks_per_vertex_prefixSum[total_vertices] = *retained_edge_blocks;
}

__global__ void compactionEBCKernel1(unsigned int *total_blocks, unsigned int total_vertices,
                                     struct vertex_dictionary_structure *device_vertex_dictionary,
                                     unsigned int *freed_edge_blocks_per_vertex_prefixSum,
                                     unsigned int *swapping_edges,
                                     unsigned int *edge_count_per_vertex_prefixSum,
                                     unsigned int *required_blocks_per_vertex,
                                     unsigned int *edge_block_count_per_vertex){
    unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
    if(id >= *total_blocks) return;
    ...
    while(start <= end){
        ...
        if(freed_edge_blocks_per_vertex_prefixSum[mid] == id){
            src = mid;
            start = mid + 1;
        }
        else if(freed_edge_blocks_per_vertex_prefixSum[mid] < id){
            src = mid;
            start = mid + 1;
        }
        else end = mid - 1;
...
//    return;
    ...
    // 2. Find the edge block
    unsigned int retained_blocks_for_src = required_blocks_per_vertex[src];
    unsigned int first_thread_of_src = freed_edge_blocks_per_vertex_prefixSum[src];
    unsigned int local_thread_idx = id - first_thread_of_src;
//    unsigned int freed_blocks = freed_edge_blocks_per_vertex_prefixSum[src + 1] - freed_edge_blocks_per_vertex_prefixSum[src];
//    unsigned int local_offset = id - freed_edge_blocks_per_vertex_prefixSum[src];
    unsigned int edge_block_offset = local_thread_idx + retained_blocks_for_src;
    unsigned long bitString = bit_string_lookup[edge_block_offset];
    struct edge_block *req_edge_block = traverse_bit_string(root, bitString);


    if(edge_block_count_per_vertex[src] <= edge_block_offset) printf("%u %u %u %u \n", id, src, edge_block_count_per_vertex[src], edge_block_offset);
//    return;

    // 3. Push the valid edges to the global array
    unsigned int e, push_index;
    for(unsigned int i = 0; i < EDGE_BLOCK_SIZE; ++i){
        e = (unsigned int) req_edge_block->edge_block_entry[i].destination_vertex;
        if (e == INFTY) continue;
        if(e == 0) break;

        // Found a valid edge
        push_index = atomicAdd(&edge_count_per_vertex_prefixSum[src], 1);
        swapping_edges[push_index] = e;
    }

    // 4. Free the edge block and remove parent to child relation
    unsigned int push_index_for_edge_queue = atomicAdd(&d_e_queue.rear, 1);
    // push_index_for_edge_queue %= EDGE_PREALLOCATE_LIST_SIZE;
    push_index_for_edge_queue %= d_e_queue.size;
    unsigned long parent_bit_string;
    struct edge_block *parent;
    if(edge_block_offset > 0) {
        if (edge_block_offset & 1) {
            parent_bit_string = bit_string_lookup[edge_block_offset / 2];
            parent = traverse_bit_string(root, parent_bit_string);
            parent->lptr = NULL;
        } else {
            parent_bit_string = bit_string_lookup[edge_block_offset / 2 - 1];
            parent = traverse_bit_string(root, parent_bit_string);
            parent->rptr = NULL;
        }
    }
...
__global__ void compactionEBCKernel2(unsigned int total_vertices, unsigned int *total_blocks,
                                     unsigned int *required_edge_blocks_per_vertex_prefixSum,
                                     unsigned int *per_vertex_edge_count,
                                     unsigned int *per_edge_block_need,
                                     struct vertex_dictionary_structure *device_vertex_dictionary,
                                     unsigned int *swapping_edges){
    unsigned int id = blockDim.x * blockIdx.x + threadIdx.x;
    if(id >= *total_blocks) return;
    ...
    while(start <= end){
        ...
        if(required_edge_blocks_per_vertex_prefixSum[mid] == id){
            src = mid;
            start = mid + 1;
        }
        else if(required_edge_blocks_per_vertex_prefixSum[mid] < id){
            src = mid;
            start = mid + 1;
        }
        else end = mid - 1;
    ...
    // 2. Find the edge block
    unsigned int edge_block_offset = id - required_edge_blocks_per_vertex_prefixSum[src];
    unsigned long bitString = bit_string_lookup[edge_block_offset];
    struct edge_block *req_edge_block = traverse_bit_string(root, bitString);

    unsigned int e;
    unsigned int swap_edges_start_for_src = per_vertex_edge_count[src];
    unsigned int edge_block_start_for_src = required_edge_blocks_per_vertex_prefixSum[src];
    unsigned int need_for_first_edge_block_of_src = per_edge_block_need[edge_block_start_for_src];
    unsigned int swap_start_for_req_block = per_edge_block_need[id] - need_for_first_edge_block_of_src + swap_edges_start_for_src;
    unsigned int last_edge_block_of_src = required_edge_blocks_per_vertex_prefixSum[src + 1] - 1;
    ...
    for(unsigned int i = 0; i < EDGE_BLOCK_SIZE; ++i){
        ...
        e = swapping_edges[swap_edges_start_for_src];
        req_edge_block->edge_block_entry[i].destination_vertex = e;
        swap_edges_start_for_src++;

        if(e == 0){
            device_vertex_dictionary->last_insert_edge_block[src] = req_edge_block;
            device_vertex_dictionary->last_insert_edge_offset[src] = i;
            zeroFound = 1;
            break;
        }
    }

    if(id == last_edge_block_of_src && zeroFound == 0){
        device_vertex_dictionary->last_insert_edge_block[src] = req_edge_block;
        device_vertex_dictionary->last_insert_edge_offset[src] = 0;
    }
}

__global__ void findPerEdgeBlockNeedOfEdges(unsigned int total_vertices, unsigned int *total_blocks,
                                            struct vertex_dictionary_structure *device_vertex_dictionary,
                                            unsigned int *required_edge_blocks_per_vertex_prefixSum,
                                            unsigned int *per_edge_block_need){
    unsigned int id = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    // 2. For each edge block find the need of edges (or infinity entries)
    unsigned int e;
    ...
    per_edge_block_need[id] = count;
}

__global__ void singleArrayPrefixSum(unsigned int n, unsigned int *arr,
                                     unsigned int *prefixSum,
                                     unsigned int *totalPrefixSum){
    unsigned int sum = arr[n - 1] + prefixSum[n - 1];
    *totalPrefixSum = sum;
    prefixSum[n] = sum;
}

__global__ void printarrebc(unsigned int n, unsigned int *arr){
    for(unsigned int i = 0; i < n; ++i) printf("%u ", arr[i]);
    printf("\n");
}

__global__ void copyArr(unsigned int n, unsigned int *from, unsigned int *to){
    unsigned int id = blockIdx.x *blockDim.x + threadIdx.x;
    if(id >= n) return;
    to[id] = from[id];
}

void compactionNew(struct vertex_dictionary_structure *device_vertex_dictionary, struct graph_properties *h_graph_prop, double fullTime){
    // 1. Find active edge count per vertex and Find edge block count per vertex
    clock_t start, end;
    clock_t prefix_start, prefix_end;
    float total_preprocess_time = 0.0;
    float tc_kernel_time = 0.0;
    ...
    std::cout << "PREPROCESSING BEFORE LAUNCHING COMPACTION" << std::endl;
    ...
    cudaMalloc(&edge_count_per_vertex, total_vertices * sizeof(unsigned int));
    cudaMalloc(&edge_block_count_per_vertex, total_vertices * sizeof(unsigned int));
    cudaMalloc(&freed_edge_blocks_per_vertex, total_vertices * sizeof(unsigned int));
    ...
    // 2. Find required blocks per vertex
    unsigned int *required_blocks_per_vertex;
    cudaMalloc(&required_blocks_per_vertex, total_vertices * sizeof(unsigned int));
    thread_blocks = ceil((double) total_vertices / THREADS_PER_BLOCK);
    prefix_start = clock();
    findRetainedBlocksAfterCompaction<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, edge_count_per_vertex, edge_block_count_per_vertex, required_blocks_per_vertex, freed_edge_blocks_per_vertex);
    cudaDeviceSynchronize();
    prefix_end = clock();
    ...
    // 3. Do prefix sum of active edge count, edge block count, req. blocks per vertex
    unsigned int *edge_count_per_vertex_prefixSum;
    unsigned int *edge_block_count_per_vertex_prefixSum;
    unsigned int *required_blocks_per_vertex_prefixSum;
    unsigned int *freed_edge_blocks_per_vertex_prefixSum;
    cudaMalloc(&edge_count_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));
    cudaMalloc(&edge_block_count_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));
    cudaMalloc(&required_blocks_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));
    cudaMalloc(&freed_edge_blocks_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int));

    thrust::device_ptr<unsigned int> edge_count_per_vertex_ptr = thrust::device_pointer_cast(edge_count_per_vertex);
    thrust::device_ptr<unsigned int> edge_count_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(edge_count_per_vertex_prefixSum);
    thrust::device_ptr<unsigned int> edge_block_count_per_vertex_ptr = thrust::device_pointer_cast(edge_block_count_per_vertex);
    thrust::device_ptr<unsigned int> edge_block_count_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(edge_block_count_per_vertex_prefixSum);
    thrust::device_ptr<unsigned int> required_blocks_per_vertex_ptr = thrust::device_pointer_cast(required_blocks_per_vertex);
    thrust::device_ptr<unsigned int> required_blocks_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(required_blocks_per_vertex_prefixSum);
    thrust::device_ptr<unsigned int> freed_edge_blocks_per_vertex_ptr = thrust::device_pointer_cast(freed_edge_blocks_per_vertex);
    thrust::device_ptr<unsigned int> freed_edge_blocks_per_vertex_prefixSum_ptr = thrust::device_pointer_cast(freed_edge_blocks_per_vertex_prefixSum);    cudaDeviceSynchronize();

    prefix_start = clock();
    thrust::exclusive_scan(thrust::device, edge_count_per_vertex_ptr, edge_count_per_vertex_ptr + total_vertices, edge_count_per_vertex_prefixSum_ptr);
    thrust::exclusive_scan(thrust::device, edge_block_count_per_vertex_ptr, edge_block_count_per_vertex_ptr + total_vertices, edge_block_count_per_vertex_prefixSum_ptr);
    thrust::exclusive_scan(thrust::device, required_blocks_per_vertex_ptr, required_blocks_per_vertex_ptr + total_vertices, required_blocks_per_vertex_prefixSum_ptr);
    thrust::exclusive_scan(thrust::device, freed_edge_blocks_per_vertex_ptr, freed_edge_blocks_per_vertex_ptr + total_vertices, freed_edge_blocks_per_vertex_prefixSum_ptr);
    cudaDeviceSynchronize();
    prefix_end = clock();
    ...
    unsigned int *host_total_active_edge_count, *host_total_edge_block_count, *host_total_freed_blocks, *host_retained_edge_blocks;
    unsigned int *device_total_active_edge_count, *device_total_edge_block_count, *device_total_freed_blocks, *device_retained_edge_blocks;
    cudaMalloc(&device_total_active_edge_count, sizeof(unsigned int));
    cudaMalloc(&device_total_edge_block_count, sizeof(unsigned int));
    cudaMalloc(&device_total_freed_blocks, sizeof(unsigned int));
    cudaMalloc(&device_retained_edge_blocks, sizeof(unsigned int));

    findTotalActiveEdgeandTotalEdgeBlocks<<<1,1>>>(total_vertices,
                                                   device_total_active_edge_count,
                                                   device_total_edge_block_count,
                                                   device_total_freed_blocks,
                                                   edge_count_per_vertex,
                                                   edge_count_per_vertex_prefixSum,
                                                   edge_block_count_per_vertex,
                                                   edge_block_count_per_vertex_prefixSum,
                                                   freed_edge_blocks_per_vertex,
                                                   freed_edge_blocks_per_vertex_prefixSum,
                                                   device_retained_edge_blocks,
                                                   required_blocks_per_vertex,
                                                   required_blocks_per_vertex_prefixSum);
    cudaDeviceSynchronize();



    host_total_active_edge_count = (unsigned int *)malloc(sizeof(unsigned int));
    host_total_edge_block_count = (unsigned int *)malloc(sizeof(unsigned int));
    host_total_freed_blocks = (unsigned int *)malloc(sizeof(unsigned int));
    host_retained_edge_blocks = (unsigned int *)malloc(sizeof(unsigned int));

    cudaMemcpy(host_total_active_edge_count, device_total_active_edge_count, sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_total_edge_block_count, device_total_edge_block_count, sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_total_freed_blocks, device_total_freed_blocks, sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_retained_edge_blocks, device_retained_edge_blocks, sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();

//    std::cout << *host_total_active_edge_count << std::endl;
//    std::cout << *host_total_edge_block_count << std::endl;
//    std::cout << *host_total_freed_blocks << std::endl;
//    std::cout << *host_retained_edge_blocks << std::endl;


    // 4. Declare a global array to store swapping edges. Size is equal to prefix sum of active edge count.
    unsigned int *swapping_edges;
    cudaMalloc(&swapping_edges, (*host_total_active_edge_count) * sizeof(unsigned int));
    unsigned int *edge_count_per_vertex_prefixSum_copy;
    cudaMalloc(&edge_count_per_vertex_prefixSum_copy, (total_vertices + 1) * sizeof(unsigned int));
    cudaMemcpy(edge_count_per_vertex_prefixSum_copy, edge_count_per_vertex_prefixSum, (total_vertices + 1) * sizeof(unsigned int), cudaMemcpyDeviceToDevice);
    cudaDeviceSynchronize();

//    thread_blocks = ceil((double) (total_vertices + 1) / THREADS_PER_BLOCK);
//    copyArr<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices + 1, edge_count_per_vertex_prefixSum, edge_count_per_vertex_prefixSum_copy);
//    cudaDeviceSynchronize();


//    printarrebc<<<1,1>>>(total_vertices + 1, edge_count_per_vertex_prefixSum_copy);
//    cudaDeviceSynchronize();



    // unsigned int* holesPerVertexBefore;
    // cudaMalloc(&holesPerVertexBefore, (unsigned int)total_vertices * sizeof(unsigned int));
    // thread_blocks = ceil((double)(total_vertices) / THREADS_PER_BLOCK);
    // findHolesPerVertex<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, device_vertex_dictionary, holesPerVertexBefore);
    // cudaDeviceSynchronize();

    // unsigned int* total_holes_before_compaction;
    // cudaMalloc(&total_holes_before_compaction, sizeof(unsigned int));
    // doPrefixSum<<<1,1>>>(holesPerVertexBefore, total_vertices, total_holes_before_compaction);
    // cudaDeviceSynchronize();
    ...
    // cudaMalloc(&num_blocks_before, sizeof(unsigned int));
    // cudaMalloc(&totalMemoryBeforeCompaction, sizeof(double));
    // recoveredMemoryKernel1<<<1,1>>>(num_blocks_before, totalMemoryBeforeCompaction, device_total_edge_block_count, device_vertex_dictionary);
    // cudaDeviceSynchronize();

    std::cout << "PREPROCESSING BEFORE LAUNCHING COMPACTION DONE!!" << std::endl;

    // 5. Launch ebc kernel. Do processing only for to be freed blocks. Take valid edges and push it to global array.
    thread_blocks = ceil((double) *host_total_freed_blocks / THREADS_PER_BLOCK);
    start = clock();
    compactionEBCKernel1<<<thread_blocks, THREADS_PER_BLOCK>>>(device_total_freed_blocks,
                                                               total_vertices,
                                                               device_vertex_dictionary,
                                                               freed_edge_blocks_per_vertex_prefixSum,
                                                               swapping_edges,
                                                               edge_count_per_vertex_prefixSum_copy,
                                                               required_blocks_per_vertex,
                                                               edge_block_count_per_vertex);
    cudaDeviceSynchronize();
    end = clock();
...
//    printarrebc<<<1,1>>>(*host_total_active_edge_count, swapping_edges);
//    cudaDeviceSynchronize();


    // 6. Find per edge block need for the edges
    unsigned int *per_edge_block_need;
    cudaMalloc(&per_edge_block_need, *host_retained_edge_blocks * sizeof(unsigned int));
    thread_blocks = ceil((double) *host_retained_edge_blocks / THREADS_PER_BLOCK);
    prefix_start = clock();
    findPerEdgeBlockNeedOfEdges<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, device_retained_edge_blocks, device_vertex_dictionary, required_blocks_per_vertex_prefixSum, per_edge_block_need);
    cudaDeviceSynchronize();
    prefix_end = clock();
    total_preprocess_time += ((double) (prefix_end - prefix_start)) / CLOCKS_PER_SEC * 1000;

//    printarrebc<<<1,1>>>(*host_retained_edge_blocks, per_edge_block_need);
//    cudaDeviceSynchronize();


    unsigned int *per_edge_block_need_prefixSum;
    cudaMalloc(&per_edge_block_need_prefixSum, (*host_retained_edge_blocks + 1) * sizeof(unsigned int));

    thrust::device_ptr<unsigned int> per_edge_block_need_ptr = thrust::device_pointer_cast(per_edge_block_need);
    thrust::device_ptr<unsigned int> per_edge_block_need_prefixSum_ptr = thrust::device_pointer_cast(per_edge_block_need_prefixSum);

    prefix_start = clock();
    thrust::exclusive_scan(thrust::device, per_edge_block_need_ptr, per_edge_block_need_ptr + *host_retained_edge_blocks, per_edge_block_need_prefixSum_ptr);
    cudaDeviceSynchronize();
    prefix_end = clock();

    total_preprocess_time += ((double) (prefix_end - prefix_start)) / CLOCKS_PER_SEC * 1000;

//    printarrebc<<<1,1>>>(*host_retained_edge_blocks + 1, per_edge_block_need_prefixSum);
//    cudaDeviceSynchronize();


    unsigned int *host_total_edge_need;
    host_total_edge_need = (unsigned int *)malloc(sizeof(unsigned int));
    unsigned int *device_total_edge_need;
    cudaMalloc(&device_total_edge_need, sizeof(unsigned int));
    unsigned int temp_n = *host_retained_edge_blocks;
    singleArrayPrefixSum<<<1,1>>>(temp_n, per_edge_block_need, per_edge_block_need_prefixSum, device_total_edge_need);
    cudaDeviceSynchronize();
    cudaMemcpy(host_total_edge_need, device_total_edge_need, sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();

//    printarrebc<<<1,1>>>(temp_n + 1, per_edge_block_need_prefixSum);
//    cudaDeviceSynchronize();


    // 6. Launch ebc kernel. Do processing only for the blocks which will be retained.
    thread_blocks = ceil((double) *host_retained_edge_blocks / THREADS_PER_BLOCK);
    start = clock();
    compactionEBCKernel2<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, device_retained_edge_blocks, required_blocks_per_vertex_prefixSum, edge_count_per_vertex_prefixSum, per_edge_block_need_prefixSum, device_vertex_dictionary, swapping_edges);
    cudaDeviceSynchronize();
    end = clock();

    total_time += (double) (end - start) / CLOCKS_PER_SEC * 1000;

    std::cout << "Total preprocessing time: " << total_preprocess_time << std::endl;
    std::cout << "Total compaction kernel time: " << total_time << std::endl;
    std::cout << "Total compaction time: " << total_preprocess_time + total_time << std::endl;
    std::cout << "Compaction Overhead: " << (total_preprocess_time + total_time) / fullTime << std::endl;

    // unsigned int* holesPerVertexAfter;
    // cudaMalloc(&holesPerVertexAfter, (unsigned int)total_vertices * sizeof(unsigned int));
    // thread_blocks = ceil((double)(total_vertices) / THREADS_PER_BLOCK);
    // findHolesPerVertex<<<thread_blocks, THREADS_PER_BLOCK>>>(total_vertices, device_vertex_dictionary, holesPerVertexAfter);
    // cudaDeviceSynchronize();

    // unsigned int* total_holes_after_compaction;
    // cudaMalloc(&total_holes_after_compaction, sizeof(unsigned int));
    // doPrefixSum<<<1,1>>>(holesPerVertexAfter, total_vertices, total_holes_after_compaction);
    // cudaDeviceSynchronize();

    // unsigned int *num_blocks_after;
    // cudaMalloc(&num_blocks_after, sizeof(unsigned int));
    // recoveredMemoryPercentage<<<1,1>>>(num_blocks_before, num_blocks_after, device_total_edge_block_count, device_vertex_dictionary, total_vertices, totalMemoryBeforeCompaction, total_holes_before_compaction, total_holes_after_compaction);
    // cudaDeviceSynchronize();

}

__global__ void init(unsigned int src, unsigned int *curr){
    curr[0] = src;
}
...
__device__ int findTotalIntersections(struct vertex_dictionary_structure device_vertex_dictionary,
                                            struct edge_block *root1, struct edge_block *root2,
                                            unsigned int u, unsigned int v){
    unsigned int blockCount1 = device_vertex_dictionary.edge_block_count[u];
    unsigned int blockCount2 = device_vertex_dictionary.edge_block_count[v];

    struct edge_block *curr1 = root1;
    struct edge_block *curr2 = root2;

    unsigned int curridx1 = 0;
    unsigned int curridx2 = 0;

    while(curridx1 < )
}

__global__ void tcWorklistKernel(unsigned int workers,
                                 struct vertex_dictionary_structure device_vertex_dictionary,
                                 unsigned int *curr, unsigned int *next,
                                 unsigned int *tc,
                                 float *idx,
    ...
    unsigned int u = curr[id];
    if(visited[u] == 1) return;

    struct edge_block *root = device_vertex_dictionary.edge_block_address[u];
    struct edge_block *curr_edge_block = root;
    unsigned int second_vertex, third_vertex;

    unsigned int curr_idx = 0;
    unsigned long bitString;
    unsigned int total_blocks = device_vertex_dictionary.edge_block_count[u];
    while(curr_idx < total_blocks){
        if(curr_idx > 0){
            bitString = bit_string_lookup[curr_idx];
            curr_edge_block = traverse_bit_string(root, bitString);
        }

        for(unsigned int i = 0; i < EDGE_BLOCK_SIZE; ++i){
            second_vertex = (unsigned int)curr_edge_block->edge_block_entry[i].destination_vertex;
            if(second_vertex == INFTY) continue;
            if(second_vertex == 0) return;


            struct edge_block *root2 = device_vertex_dictionary.edge_block_address[second_vertex - 1];
            unsigned int total_intersections = findTotalIntersections();
        }
    }
}

void tcWorklist(struct vertex_dictionary_structure device_vertex_dictionary, struct graph_properties *h_graph_prop){
    unsigned int total_vertices = h_graph_prop->xDim;

    unsigned int *visited;
    cudaMalloc(&visited, total_vertices * sizeof(unsigned int));

    float totatTime = 0.0;
    float time;

    unsigned int *tc;
    cudaMalloc(&tc, total_vertices * sizeof(unsigned int));

    unsigned int src_vertex = 0;

    float *workers = (float*)malloc(sizeof(float));
    *workers = 1;

    unsigned int *curr;
    cudaMalloc(&curr, (3 * total_vertices) * sizeof(unsigned int));

    unsigned int *next;
    cudaMalloc(&next, (3 * total_vertices) * sizeof(unsigned int));

    std::cout << "Initialized current worklist" << std::endl;
    std::cout << std::endl;

    float *idx;
    cudaMalloc(&idx, sizeof(float));

    std::cout << "Defined index for next worklist" << std::endl;
    std::cout << std::endl;

    init<<<1,1>>>(src_vertex, curr);
    cudaDeviceSynchronize();

    unsigned int itr = 1;
    unsigned int worklist_blocks;
...
//      char fileLoc[50] = "../../Graphs/co-papers-dblp.mtx";
    // char fileLoc[50] = "./MTX/co-papers-citeseer.mtx";
    // char fileLoc[50] = "./MTX/hugetrace-00020.mtx";
    // char fileLoc[50] = "./MTX/channel-500x100x100-b050.mtx";
    // char fileLoc[30] = "kron_g500-logn16.mtx";
    // char fileLoc[30] = "kron_g500-logn17.mtx";
    // char fileLoc[50] = "../iitrpr/Graphs_MTX/MTX/kron_g500-logn21_0.mtx";
    // char fileLoc[30] = "delaunay_n22.mtx";
    // char fileLoc[30] = "delaunay_n23.mtx";
    // char fileLoc[50] = "./MTX/delaunay_n24.mtx";
//    char fileLoc[50] = "./MTX/inf-europe_osm_4.mtx";
    // char fileLoc[30] = "rgg_n_2_23_s0.mtx";
    // char fileLoc[50] = "./MTX/rgg_n_2_24_s0.mtx";
    // char fileLoc[50] = "./MTX/nlpkkt240.mtx";
    // char fileLoc[50] = "./MTX/uk-2005_4.mtx";
    // char fileLoc[30] = "twitter7.mtx";
    // char fileLoc[50] = "./MTX/sk-2005.mtx";
    ...
    memory_usage();
    unsigned long edge_block_count_device = get_edge_block_count_device();
    cudaDeviceSynchronize();
    ...
    // edge_block_count_device = 30;

    temp_time = clock();
    struct edge_block *device_edge_block;
    struct edge_block **d_queue_edge_block_address;
    // cudaMalloc((struct edge_block**)&device_edge_block, total_edge_blocks_count_init * sizeof(struct edge_block));
    cudaMalloc((struct edge_block**)&device_edge_block, edge_block_count_device * sizeof(struct edge_block));
    cudaMalloc((struct edge_block **)&d_queue_edge_block_address, edge_block_count_device * sizeof(struct edge_block *));
    ...
    thrust::device_vector <unsigned long> d_source_vector(vertex_size + 1);
    ...
    data_structure_init<<< 1, 1>>>(device_vertex_dictionary, d_vertex_id, d_edge_block_count_VD, d_active_edge_count_VD, d_last_insert_edge_offset, d_last_insert_edge_block, d_edge_block_address, d_queue_edge_block_address, edge_block_count_device);
    ...
    thread_blocks = ceil(double(edge_block_count_device) / THREADS_PER_BLOCK);
    ...
    parallel_push_edge_preallocate_list_to_device_queue_v1<<< thread_blocks, THREADS_PER_BLOCK>>>(device_edge_block, edge_block_count_device, edge_block_count_device);
    ...
    batched_edge_inserts_EC_postprocessing<<< thread_blocks, THREADS_PER_BLOCK>>>(device_vertex_dictionary, vertex_size, edge_size, d_csr_offset_new_pointer, d_csr_edges_new_pointer, d_source_degrees_new_pointer, d_prefix_sum_edge_blocks_new_pointer, 0);
    ...
    unsigned long part_1_memory = (3 * vertex_dictionary_size * sizeof(unsigned long)) /1024/1024;
    unsigned long part_2_memory = (vertex_dictionary_size * sizeof(unsigned int)) /1024/1024;
    unsigned long part_3_memory = (2 * vertex_dictionary_size * sizeof(struct edge_block *)) /1024/1024;
    unsigned long edge_blocks_memory = (total_edge_blocks_count_batch * sizeof(struct edge_block)) /1024/1024;

    std::cout << "P1, P2, P3, EBM, edge_blocks : " << part_1_memory << ", " << part_2_memory << ", " << part_3_memory << ", " << edge_blocks_memory << ", " << total_edge_blocks_count_batch << std::endl;
    std::cout << "GraphVine memory consumption on bulk-build : " << part_1_memory + part_2_memory + part_3_memory + edge_blocks_memory << " MB"  << std::endl;
    ...
    if(choice == 2) {
        ...
        // generate_random_batch_skewed(h_graph_prop->xDim, BATCH_SIZE, h_source, h_destination, h_source_degrees_new, h_source_degrees, kk);
        std::cout << "Generated random batch" << std::endl;
        cudaDeviceSynchronize();
        ...
        if((EDGE_BLOCK_SIZE > 35) && (BATCH_SIZE <= 10000000)) {
            // if((average_degree_batch > 35) && (BATCH_SIZE <= 10000000)) {
        ...
        batched_edge_inserts_EC_postprocessing<<< thread_blocks, THREADS_PER_BLOCK>>>(device_vertex_dictionary, vertex_size, batch_size, d_csr_offset_new_pointer, d_csr_edges_new_pointer, d_source_degrees_new_pointer, d_prefix_sum_edge_blocks_new_pointer, kk);
    ...
    while(exitFlag) {
        ...
        switch(menuChoice) {
            ...
            case 4  :
            {
                ...
                unsigned long pr_choice;
                std::cout << "Enter type of PR required" << std::endl << "1. High-diameter PR (Low-degree graphs)" << std::endl << "2. Low-diameter PR (High-degree graphs)" << std::endl;
                std::cin >> pr_choice;
                pageRank_time = clock();
                ...
                if(pr_choice == 1) {
                    ...
                    // below is edge-block-centric
                    unsigned long thread_count_EBC;
                    thread_blocks = ceil(double(vertex_size) / THREADS_PER_BLOCK);
                    pageRank_kernel_preprocessing_v2<<< thread_blocks, THREADS_PER_BLOCK>>>(device_vertex_dictionary, vertex_size, d_csr_offset_new_pointer, d_prefix_sum_edge_blocks_new_pointer, d_source_degrees_new_pointer, d_source_vector_pointer);
                    cudaDeviceSynchronize();
                    ...
                    // std::cout << "Threads launched is " << thread_count_EBC << std::endl;
                    thread_blocks = ceil(double(thread_count_EBC) / THREADS_PER_BLOCK);
                    pageRank_kernel_v2<<< thread_blocks, THREADS_PER_BLOCK>>>(device_vertex_dictionary, vertex_size, d_pageRankVector_1_pointer, d_pageRankVector_2_pointer, d_source_vector_pointer);
                }
                cudaDeviceSynchronize();
                pageRank_time = clock() - pageRank_time;
                // thread_blocks = ceil(double(vertex_size) / 256);

                print_pageRank_values<<< 1, 1>>>(d_pageRankVector_2_pointer, vertex_size);
                std::cout << "PageRank Time    : " << (float)pageRank_time/CLOCKS_PER_SEC << " seconds" << std::endl;

                cudaDeviceSynchronize();

                unsigned long next_op;
                std::cout << "Enter next operation\n1. Dynamic PR\n2. Go to main menu" << std::endl;
                std::cin >> next_op;

                if(next_op == 0) {

                    unsigned long average_degree_batch, max_degree_batch, sum_degree_batch;
                    unsigned long pr_iteration = 1;


                    // insert_processing:


                    std::cout << std::endl << "Iteration " << pr_iteration << std::endl;
            ...
            case 6  :
            {
                ...
                if(sssp_type == 1) {
                    ...
                    if(next_op == 1) {
                        ...
                        for(unsigned long tt = 0 ; tt < 5 ; tt++) {
                            ...
                            // thrust::fill(d_pageRankVector_1.begin(), d_pageRankVector_1.end(), 0.25f);
                            // sssp_kernel_preprocessing<<< 1, 1>>>(d_shortest_path_pointer, d_search_flag, d_mutex_pointer, d_sssp_queue_1_pointer);
                            sssp_kernel_preprocessing_dynamic_stage1<<< 1, 1>>>(batch_size);
                            thread_blocks = ceil(double(batch_size) / THREADS_PER_BLOCK);
                            sssp_kernel_preprocessing_dynamic<<< thread_blocks, THREADS_PER_BLOCK>>>(d_shortest_path_pointer, d_search_flag, d_mutex_pointer, d_sssp_queue_1_pointer, batch_size, d_source_pointer);
                            h_dp_thread_count = batch_size;
            ...
            case 10 :
                ...
                break;
            case 11:
                triangleCountSorted(device_vertex_dictionary, h_graph_prop);
                break;
            case 12:
                fullTime += ((double) init_time / CLOCKS_PER_SEC * 1000);
                compactionNew(device_vertex_dictionary, h_graph_prop, fullTime);
            default :;
