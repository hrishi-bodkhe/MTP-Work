...
int main(){
    ...
    Node** hadjList;
    hadjList = (Node **)malloc(vertices * sizeof(Node *));
    cudaMemcpy(hadjList, dadjList, index.size() * sizeof(Node*), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    // print
    // for(ll i = 0; i < vertices; ++i){
    //     printf("%ld: ", i);
    //     Node* temp = hadjList[i];
