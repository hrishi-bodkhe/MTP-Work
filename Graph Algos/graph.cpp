...
int main(){
    ...
    else sort(edgeList.begin(), edgeList.end(), comp_Edges);
...
void constructCSR(ll& vertices, vector<ll>& index, vector<ll>& headvertex, vector<ll>& weights, int directed, int weighted, vector<Edge>& edgeList){
    ...
    for(ll i = 1; i < vertices + 1; ++i) index[i] += index[i - 1];
...
void printCSR(vector<ll>& index, vector<ll>& headvertex, vector<ll>& weights){
    ...
    cout << "Head Vertex: ";
    for(auto& v: headvertex) cout << v << ' ';
    cout << endl;

    cout << "Weights: ";
    for(auto& v: weights) cout << v << ' ';
    cout << endl;
}
...
void printEdgeList(vector<Edge>& edgeList){
    for(Edge e: edgeList) cout << e.src << ' ' << e.dest << ' ' << e.wt << endl;
}