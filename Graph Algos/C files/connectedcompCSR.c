...
void traverseConnComp(int u, int *index, int *headVertex, int color[], int *compNum, int component[])
{
	color[u] = 1;
	component[u] = *compNum;
...
void findConnCompCSR(int src, int vertices, int *index, int *headVertex)
{
	// 0 - white
	// 1 - grey
	// 2 - black
	...
	int count = 0;
	int color[vertices], component[vertices];
...
int main()
{
	...
	if (directed == 1)
	{
		...
		if (weighted == 1)
			weight = (int *)malloc(edges * sizeof(int));
	}
	else
	{
		headVertex = (int *)malloc(2 * edges * sizeof(int));
		if (weighted == 1)
			weight = (int *)malloc(2 * edges * sizeof(int));
	}
	...
	findConnCompCSR(0, vertices, index, headVertex);
